...
prediction_data_set = open("prediction_data_set.tsv", "w+")
...
multi_choice_data_set = open("multi_choice_data_set.tsv", "w+")
...
dict = api.get_paragraphs_id(books=book_ids, sents_num=(2, 100),
                             output_local_id=True, is_analysed=True)  # it can be changed depend on usage

for id in book_ids:
    temp_dict = {id: dict[id]}
    text, _ = api.get_paragraph_text(dict)
